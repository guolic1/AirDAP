...
| 网络凭据 | 导入现有 JSON，或通过 BLE 将服务保存的凭据写入设备；重复配对复用本机凭据 |
...
2. 已有凭据可直接导入；否则开启设备 BLE 配网窗口，在页面点击蓝牙配对。不同凭据会撤销其他主机的旧网络会话。
3. 通过 USB 或 BLE 配置 Wi-Fi；USB 需要固件启用独立 debug shell，仅支持 ASCII SSID/密码。
4. 确认设备 IP。mDNS 无法解析时填写实际 IP。点击网络读取验证，再启动桥接。
5. 虚拟 USB 序列号为 `ADP-xxxxxxxxxxxx-NET`；目标 COM 编号由系统分配。配置、配对、升级前先停止桥接。

USB 配网不会生成网络 PSK。BLE 配网和 BLE 配对是两个明确操作。
...
上传只在内存暂存一份镜像，不写设备。页面显示 ESP32-S3 应用描述符版本、大小和 SHA-256；
