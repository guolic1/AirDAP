...
for suite in \
    ...
    debug_shell_tx_state airdap_shell airdap_update wired_hil; do
...
transitions, stale USB-frame recovery, host update ordering, UART line-coding mapping, both compile-time
USB descriptor variants, bounded shell input, and the debug TX completion
state, host tools, and wired HIL helper's protocol checks. They do not prove
USB enumeration, physical flash persistence, bootloader rollback on a board,
or electrical SWD timing.
