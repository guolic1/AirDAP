...
static uint8_t commands[5] = {0, 12, 2, 0, 3};
...
{
    const uint8_t defaults[5] = {0, 12, 2, 0, 3};
...
const char *airdap_button_command_name(airdap_button_command_t command)
{
    const char *names[] = {"none", "dap-toggle", "provisioning", "clear-network-restart", "dap-usb", "dap-network", "dap-auto",
        "restart", "target-reset", "wifi-toggle", "target-power-toggle", "target-power-cycle", "dap-network-auto-toggle"};
...
airdap_mode_dap_result_t airdap_mode_state_set_dap_route(airdap_dap_route_t next)
{
    ...
    if (next == AIRDAP_DAP_ROUTE_NETWORK_AUTO_TOGGLE) {
        route = route == AIRDAP_DAP_ROUTE_NETWORK ? AIRDAP_DAP_ROUTE_AUTO : AIRDAP_DAP_ROUTE_NETWORK;
