...
int main(void)
{
    ...
    const uint8_t expected[5] = {0, 12, 2, 0, 3};
    ...
    assert(airdap_button_config_init() == ESP_OK);
    assert(airdap_button_config_get(commands) == ESP_OK && memcmp(commands, expected, 5) == 0);
    /* Upgrade old defaults once, preserving other customized gestures. */
    airdap_button_config_test_reset();
    const uint8_t legacy[6] = {1, 4, 1, 2, 11, 3};
    memcpy(durable, legacy, sizeof(legacy));
    commit_error = ESP_FAIL;
    assert(airdap_button_config_init() == ESP_FAIL && !handle_open);
    assert(airdap_button_config_get(commands) == ESP_ERR_INVALID_STATE);
    assert(memcmp(durable, legacy, sizeof(legacy)) == 0);
    commit_error = ESP_OK;
    assert(airdap_button_config_init() == ESP_OK);
    assert(airdap_button_config_get(commands) == ESP_OK);
    assert(commands[0] == 4 && commands[1] == 12 && commands[3] == 11 && durable[0] == 2);
    assert(airdap_button_config_set(AIRDAP_BUTTON_GESTURE_DOUBLE, AIRDAP_BUTTON_COMMAND_DAP_TOGGLE) == ESP_OK);
    airdap_button_config_test_reset();
    const unsigned before_reload = writes;
    assert(airdap_button_config_init() == ESP_OK && writes == before_reload);
    assert(airdap_button_config_get(commands) == ESP_OK && commands[1] == 1);
    airdap_button_config_test_reset();
    durable[0] = 1;
    durable[2] = AIRDAP_BUTTON_COMMAND_TARGET_RESET;
    assert(airdap_button_config_init() == ESP_OK);
    assert(airdap_button_config_get(commands) == ESP_OK && commands[1] == AIRDAP_BUTTON_COMMAND_TARGET_RESET);
